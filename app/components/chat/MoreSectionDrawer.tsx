...
export default function MoreSectionDrawer({ isOpen, onClose, onSendMessage }: MoreSectionDrawerProps) {
  ...
  const handleQuestionClick = (question: string, id: string) => {
    ...
    // Show selection briefly then close
    setTimeout(() => {
      onClose()
    }, 600)
  }
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative flex w-full max-w-md flex-col bg-white/90 backdrop-blur-xl border border-white/20 shadow-2xl rounded-t-3xl overflow-hidden"
          ...
          >
            ...
            <button
              className="absolute right-4 top-4 z-10 w-8 h-8 rounded-full bg-gray-100/80 backdrop-blur-sm border border-gray-200/50 flex items-center justify-center transition-all duration-200 hover:bg-gray-200/80 hover:scale-105 active:scale-95"
              onClick={onClose}
            >
            ...
            <div className="px-6 pb-2">
              <motion.h2
                ...
                className="text-3xl font-bold text-gray-900 tracking-tight -letter-spacing-0.5"
              ...
              <div className="space-y-8">
                {moreQuestions.map((categoryGroup, categoryIndex) => {
                  ...
                  return (
                    ...
                    >
                      {/* Category Header - Apple Style with Dynamic Icons */}
